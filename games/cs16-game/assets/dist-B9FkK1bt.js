//#region node_modules/xash3d-fwgs/dist/constants.js
var DEFAULT_XASH_LIBRARY = "xash.wasm";
var DEFAULT_FILESYSTEM_LIBRARY = "filesystem_stdio.wasm";
var DEFAULT_GLES3COMPAT_LIBRARY = "libref_webgl2.wasm";
var DEFAULT_GL4ES_LIBRARY = "libref_webgl2.wasm";
var DEFAULT_SOFT_LIBRARY = "libref_soft.wasm";
var DEFAULT_MENU_LIBRARY = "cl_dlls/menu_emscripten_wasm32.wasm";
var DEFAULT_CLIENT_LIBRARY = "cl_dlls/client_emscripten_wasm32.wasm";
var DEFAULT_SERVER_LIBRARY = "dlls/hl_emscripten_wasm32.wasm";
//#endregion
//#region node_modules/xash3d-fwgs/dist/generated/xash.js
var Xash3D$1 = (() => {
	...
	return async function(moduleArg = {}) {
		...
		function getWasmImports() {
			var GOTProxyHandler = new Proxy(new Set([
		...
		var GOT = {};
		var GOTHandler = { get(weakImports, symName) {
			var rtn = GOT[symName];
			if (!rtn) rtn = GOT[symName] = new WebAssembly.Global({
				value: "i32",
				mutable: true
			}, -1);
			if (!weakImports.has(symName)) rtn.required = true;
			return rtn;
		} };
		...
		var getDylinkMetadata = (binary) => {
			...
			if (binary instanceof WebAssembly.Module) {
				...
				failIf(!(new Uint32Array(new Uint8Array(binary.subarray(0, 24)).buffer)[0] == 1836278016), "need to see wasm magic number");
				failIf(binary[8] !== 0, "need the dylink section to be first");
				offset = 9;
				var section_size = getLEB();
				end = offset + section_size;
				failIf(getString() !== "dylink.0");
			}
			var customSection = {
				neededDynlibs: [],
			...
			while (offset < end) {
				...
				} else if (subsectionType === WASM_DYLINK_NEEDED) customSection.neededDynlibs = getStringList();
				...
				} else if (subsectionType === WASM_DYLINK_RUNTIME_PATH) customSection.runtimePaths = getStringList();
		...
		var getMemory = (size) => {
			if (runtimeInitialized) return _calloc(size, 1);
		...
		var isImmutableGlobal = (val) => {
			if (val instanceof WebAssembly.Global) try {
				val.value = val.value;
			} catch {
				return true;
			}
		...
		var relocateExports = (exports, memoryBase = 0) => {
			function relocateExport(name, value) {
				if (isImmutableGlobal(value)) return new WebAssembly.Global({ value: "i32" }, value.value + memoryBase);
				return value;
			}
			var relocated = {};
			for (var e in exports) relocated[e] = relocateExport(e, exports[e]);
			return relocated;
		};
		...
		var resolveGlobalSymbol = (symName, direct = false) => {
			...
			else if (symName.startsWith("invoke_")) sym = wasmImports[symName] = createNamedFunction(symName, createInvokeFunction(symName.split("_")[1]));
			return {
				sym,
				name: symName
			};
		...
		var loadWebAssemblyModule = (binary, flags, libName, localScope, handle) => {
			...
			function loadModule() {
				...
				if (flags.loadAsync) return (async () => {
					var instance;
					if (binary instanceof WebAssembly.Module) instance = new WebAssembly.Instance(binary, info);
					else ({module: binary, instance} = await WebAssembly.instantiate(binary, info));
			...
			if (flags.loadAsync) return metadata.neededDynlibs.reduce((chain, needed) => chain.then(() => {
		...
		var registerWasmPlugin = () => {
			var wasmPlugin = {
				...
				handle: async (byteArray, name) => wasmPlugin.promiseChainEnd = wasmPlugin.promiseChainEnd.then(async () => {
					try {
		...
		function loadDynamicLibrary(libName, flags = {
			...
			function loadLibData() {
				...
				if (flags.loadAsync) return asyncLoad(libFile);
			...
			function getExports() {
				...
				if (preloaded) return flags.loadAsync ? Promise.resolve(preloaded) : preloaded;
			...
			function moduleLoaded(exports) {
				if (dso.global) mergeLibSymbols(exports, libName);
				else if (localScope) Object.assign(localScope, exports);
				dso.exports = exports;
			}
			if (flags.loadAsync) return getExports().then((exports) => {
		...
		var reportUndefinedSymbols = () => {
			for (var [symName, entry] of Object.entries(GOT)) if (entry.value == -1) {
				var value = resolveGlobalSymbol(symName, true).sym;
				if (!value && !entry.required) {
					entry.value = 0;
					continue;
				}
				if (typeof value == "function") entry.value = addFunction(value, value.sig);
				else if (typeof value == "number") entry.value = value;
				else if (typeof value.value == "number") entry.value = value;
				else throw new Error(`bad export type for '${symName}': ${typeof value} (${value})`);
			}
		};
		var loadDylibs = async () => {
			...
			for (var lib of dynamicLibraries) await loadDynamicLibrary(lib, {
				loadAsync: true,
				global: true,
				nodelete: true,
				allowUndefined: true
		...
		var SOCKFS = {
			...
			createSocket(family, type, protocol) {
				if (family != 2) throw new FS.ErrnoError(5);
				type &= -526337;
				if (type != 1 && type != 2) throw new FS.ErrnoError(28);
				if (type == 1 && protocol && protocol != 6) throw new FS.ErrnoError(66);
				var sock = {
					family,
					type,
					protocol,
			...
			websocket_sock_ops: {
				createPeer(sock, addr, port) {
					...
					if (ws) if (ws._socket) {
						...
						if (SOCKFS.websocketArgs["subprotocol"]) subProtocols = SOCKFS.websocketArgs["subprotocol"];
						else if (SOCKFS.websocketArgs["subprotocol"] === null) subProtocols = "null";
						if (url === "ws://" || url === "wss://") {
					...
					if (sock.type === 2 && typeof sock.sport != "undefined") peer.msg_send_queue.push(new Uint8Array([
						255,
						255,
						255,
						255,
				...
				poll(sock) {
					...
					if (dest && dest.socket.readyState === dest.socket.CLOSING || dest && dest.socket.readyState === dest.socket.CLOSED) if (sock.connecting) mask |= 4;
					else mask |= 16;
					return mask;
				},
				ioctl(sock, request, arg) {
					switch (request) {
						case 21531:
							...
							return 0;
						case 21537:
							if (HEAP32[arg >> 2]) sock.stream.flags |= 2048;
							else sock.stream.flags &= -2049;
							return 0;
						default: return 28;
					}
				},
				...
				connect(sock, addr, port) {
					...
					if (typeof sock.daddr != "undefined" && typeof sock.dport != "undefined") {
						...
						if (dest) if (dest.socket.readyState === dest.socket.CONNECTING) throw new FS.ErrnoError(7);
				...
				getname(sock, peer) {
					var addr, port;
					if (peer) {
						if (sock.daddr === void 0 || sock.dport === void 0) throw new FS.ErrnoError(53);
						addr = sock.daddr;
						port = sock.dport;
					} else {
						addr = sock.saddr || 0;
				...
				sendmsg(sock, buffer, offset, length, addr, port) {
					if (sock.type === 2) {
						...
						if (addr === void 0 || port === void 0) throw new FS.ErrnoError(17);
					} else {
						addr = sock.daddr;
						port = sock.dport;
					}
		...
		var inetPton6 = (str) => {
			...
			if (str.indexOf(".") > 0) {
				str = str.replace(/* @__PURE__ */ new RegExp("[.]", "g"), ":");
				words = str.split(":");
			...
			offset = 0;
			z = 0;
			for (w = 0; w < words.length; w++) if (typeof words[w] == "string") if (words[w] === "Z") {
		...
		function ___syscall_fallocate(fd, mode, offset, len) {
			...
			try {
				...
				if (mode != 0) return -138;
				if (offset < 0 || len < 0) return -28;
				var oldSize = FS.fstat(fd).size;
				var newSize = offset + len;
				if (newSize > oldSize) FS.ftruncate(fd, newSize);
				return 0;
			} catch (e) {
				if (typeof FS == "undefined" || !(e.name === "ErrnoError")) throw e;
				return -e.errno;
		...
		var PIPEFS = {
			...
			stream_ops: {
				...
				poll(stream, timeout, notifyCallback) {
					...
					if ((stream.flags & 2097155) === 1) return 260;
					for (var bucket of pipe.buckets) if (bucket.offset - bucket.roffset > 0) return 65;
					return 0;
				},
				dup(stream) {
				...
				read(stream, buffer, offset, length, position) {
					var pipe = stream.node.pipe;
					var currentLength = 0;
					for (var bucket of pipe.buckets) currentLength += bucket.offset - bucket.roffset;
					var data = buffer.subarray(offset, offset + length);
					if (length <= 0) return 0;
					if (currentLength == 0) throw new FS.ErrnoError(6);
				...
				write(stream, buffer, offset, length, position) {
					...
					} else currBucket = pipe.buckets[pipe.buckets.length - 1];
				...
				close(stream) {
					var pipe = stream.node.pipe;
					pipe.refcnt--;
					if (pipe.refcnt === 0) pipe.buckets = null;
				}
			},
			nextname() {
				if (!PIPEFS.nextname.current) PIPEFS.nextname.current = 0;
				return "pipe[" + PIPEFS.nextname.current++ + "]";
			}
		};
		function ___syscall_pipe(fdPtr) {
			try {
				if (fdPtr == 0) throw new FS.ErrnoError(21);
		...
		function ___syscall_recvmsg(fd, message, flags, d1, d2, d3) {
			try {
				...
				if (name) var errno = writeSockaddr(name, sock.family, DNS.lookup_name(msg.addr), msg.port);
				var bytesRead = 0;
		...
		function ___syscall_sendmsg(fd, message, flags, d1, d2, d3) {
			try {
				...
				for (var i = 0; i < num; i++) {
					var iovbase = HEAPU32[iov + (8 * i + 0) >> 2];
					var iovlen = HEAP32[iov + (8 * i + 4) >> 2];
					for (var j = 0; j < iovlen; j++) view[offset++] = HEAP8[iovbase + j];
				}
		...
		function ___syscall_sendto(fd, message, length, flags, addr, addr_len) {
			try {
				var sock = getSocketFromFD(fd);
				if (!addr) return FS.write(sock.stream, HEAP8, message, length);
		...
		var dlopenInternal = (handle, jsflags) => {
			...
			if (jsflags.loadAsync) return loadDynamicLibrary(filename, combinedFlags, localScope, handle);
			try {
		...
		var __emscripten_dlopen_js = (handle, onsuccess, onerror, user_data) => {
			function errorCallback(e) {
				dlSetError(`'Could not load dynamic lib: ${UTF8ToString(handle + 36)}\n${e}`);
			...
			if (promise) promise.then(successCallback, errorCallback);
			else errorCallback();
		};
		...
		var JSEvents = {
			...
			deferCall(targetFunction, precedence, argsList) {
				function arraysHaveEqualContent(arrA, arrB) {
					...
					for (var i in arrA) if (arrA[i] != arrB[i]) return false;
					return true;
				}
		...
		var getCallstack = (flags) => {
			...
			callstack = "";
			var firefoxRe = /* @__PURE__ */ new RegExp("\\s*(.*?)@(.*?):([0-9]+):([0-9]+)");
			var chromeRe = /* @__PURE__ */ new RegExp("\\s*at (.*?) \\((.*):(.*):(.*)\\)");
			for (var line of lines) {
				...
				if (flags & 24) {
					if (flags & 64) file = file.substring(file.replace(/\\/g, "/").lastIndexOf("/") + 1);
		...
		var __emscripten_log_formatted = (flags, str) => {
			...
			if (flags & 1) if (flags & 4) console.error(str);
			else if (flags & 2) console.warn(str);
			else if (flags & 512) console.info(str);
			else if (flags & 256) console.debug(str);
			else console.log(str);
			else if (flags & 6) err(str);
			else out(str);
		};
		...
		var __mktime_js = function(tmPtr) {
			var ret = (() => {
				...
				if (dst < 0) HEAP32[tmPtr + 32 >> 2] = Number(summerOffset != winterOffset && dstOffset == guessedOffset);
					...
					var trueOffset = dst > 0 ? dstOffset : Math.max(winterOffset, summerOffset);
		...
		function __poll_js(fds, nfds, timeout, ctx, arg) {
			try {
				...
				for (var i = 0; i < nfds; i++) {
					...
					var flags = 32;
					var stream = FS.getStream(fd);
					if (stream) if (stream.stream_ops.poll) flags = stream.stream_ops.poll(stream, -1);
					else flags = 5;
					flags &= events | 24;
					if (flags) count++;
		...
		var AL = {
			...
			scheduleContextAudio: (ctx) => {
				if (MainLoop.timingMode === 1 && document["visibilityState"] != "visible") return;
				for (var i in ctx.sources) AL.scheduleSourceAudio(ctx.sources[i]);
			},
			scheduleSourceAudio: (src, lookahead) => {
				if (MainLoop.timingMode === 1 && document["visibilityState"] != "visible") return;
				if (src.state !== 4114) return;
				...
				if (!lookahead) lookahead = AL.QUEUE_LOOKAHEAD;
				var lookaheadTime = currentTime + lookahead;
				var skipCount = 0;
				while (startTime < lookaheadTime) {
					if (bufCursor >= src.bufQueue.length) if (src.looping) bufCursor %= src.bufQueue.length;
					else break;
					var buf = src.bufQueue[bufCursor % src.bufQueue.length];
					if (buf.length === 0) {
						skipCount++;
						if (skipCount === src.bufQueue.length) break;
					} else {
						...
						if (src.type === 4136 && src.looping) {
							...
							audioSrc.loop = true;
							if (buf.audioBuf._loopStart) audioSrc.loopStart = buf.audioBuf._loopStart;
							if (buf.audioBuf._loopEnd) audioSrc.loopEnd = buf.audioBuf._loopEnd;
						} else duration = (buf.audioBuf.duration - startOffset) / src.playbackRate;
			...
			updateSourceTime: (src) => {
				...
				if (src.state !== 4114) return currentTime;
				...
				while (src.audioQueue.length) {
					...
					if (currentTime < nextStartTime) break;
					src.audioQueue.shift();
					src.bufStartTime = nextStartTime;
					src.bufOffset = 0;
					src.bufsProcessed++;
				}
				if (src.bufsProcessed >= src.bufQueue.length && !src.looping) AL.setSourceState(src, 4116);
				else if (src.type === 4136 && src.looping) {
					var buf = src.bufQueue[0];
					if (buf.length === 0) src.bufOffset = 0;
					else {
						var delta = (currentTime - src.bufStartTime) * src.playbackRate;
						var loopStart = buf.audioBuf._loopStart || 0;
						var loopEnd = buf.audioBuf._loopEnd || buf.audioBuf.duration;
						if (loopEnd <= loopStart) loopEnd = buf.audioBuf.duration;
						if (delta < loopEnd) src.bufOffset = delta;
						else src.bufOffset = loopStart + (delta - loopStart) % (loopEnd - loopStart);
					}
				} else if (src.audioQueue[0]) src.bufOffset = (currentTime - src.audioQueue[0]._startTime) * src.playbackRate;
				else {
					if (src.type !== 4136 && src.looping) {
						var srcDuration = AL.sourceDuration(src) / src.playbackRate;
						if (srcDuration > 0) src.bufStartTime += Math.floor((currentTime - src.bufStartTime) / srcDuration) * srcDuration;
					}
					for (var i = 0; i < src.bufQueue.length; i++) {
						if (src.bufsProcessed >= src.bufQueue.length) if (src.looping) src.bufsProcessed %= src.bufQueue.length;
						else {
			...
			cancelPendingSourceAudio: (src) => {
				...
				for (var i = 1; i < src.audioQueue.length; i++) src.audioQueue[i].stop();
				if (src.audioQueue.length > 1) src.audioQueue.length = 1;
			},
			stopSourceAudio: (src) => {
				for (var i = 0; i < src.audioQueue.length; i++) src.audioQueue[i].stop();
			...
			initSourcePanner: (src) => {
				if (src.type === 4144) return;
				var templateBuf = AL.buffers[0];
				for (var i = 0; i < src.bufQueue.length; i++) if (src.bufQueue[i].id !== 0) {
					templateBuf = src.bufQueue[i];
					break;
				}
				if (src.spatialize === 1 || src.spatialize === 2 && templateBuf.channels === 1) {
					if (src.panner) return;
			...
			updateContextGlobal: (ctx) => {
				for (var i in ctx.sources) AL.updateSourceGlobal(ctx.sources[i]);
			},
			updateSourceGlobal: (src) => {
				...
				switch (src.context.sourceDistanceModel ? src.distanceModel : src.context.distanceModel) {
					case 0:
			...
			updateListenerSpace: (ctx) => {
				...
				} else listener.setPosition(ctx.listener.position[0], ctx.listener.position[1], ctx.listener.position[2]);
				...
				} else listener.setOrientation(ctx.listener.direction[0], ctx.listener.direction[1], ctx.listener.direction[2], ctx.listener.up[0], ctx.listener.up[1], ctx.listener.up[2]);
				for (var i in ctx.sources) AL.updateSourceSpace(ctx.sources[i]);
			},
			updateSourceSpace: (src) => {
				...
				} else panner.setPosition(posX, posY, posZ);
				if (panner.orientationX) {
					if (dirX != panner.orientationX.value) panner.orientationX.value = dirX;
					if (dirY != panner.orientationY.value) panner.orientationY.value = dirY;
					if (dirZ != panner.orientationZ.value) panner.orientationZ.value = dirZ;
				} else panner.setOrientation(dirX, dirY, dirZ);
				...
				if (posX === lPosX && posY === lPosY && posZ === lPosZ || velX === lVelX && velY === lVelY && velZ === lVelZ) src.dopplerShift = 1;
				else {
				...
				if (src.dopplerShift !== oldShift) AL.updateSourceRate(src);
			},
			updateSourceRate: (src) => {
				if (src.state === 4114) {
					...
					if (!audioSrc) return;
					var duration;
					if (src.type === 4136 && src.looping) duration = Number.POSITIVE_INFINITY;
					else duration = (audioSrc.buffer.duration - audioSrc._startOffset) / src.playbackRate;
					audioSrc._duration = duration;
					audioSrc.playbackRate.value = src.playbackRate;
					AL.scheduleSourceAudio(src);
				}
			...
			sourceTell: (src) => {
				AL.updateSourceTime(src);
				var offset = 0;
				for (var i = 0; i < src.bufsProcessed; i++) if (src.bufQueue[i].audioBuf) offset += src.bufQueue[i].audioBuf.duration;
				offset += src.bufOffset;
				return offset;
			},
			sourceSeek: (src, offset) => {
				var playing = src.state == 4114;
				if (playing) AL.setSourceState(src, 4113);
				...
				if (playing) AL.setSourceState(src, 4114);
			},
			getGlobalParam: (funcname, param) => {
				if (!AL.currentCtx) return null;
				switch (param) {
			...
			getSourceParam: (funcname, sourceId, param) => {
				...
				switch (param) {
					...
					case 4105:
						if (src.type === 4136) return src.bufQueue[0].id;
						return 0;
					...
					case 4117:
						if (src.bufQueue.length === 1 && src.bufQueue[0].id === 0) return 0;
						return src.bufQueue.length;
					case 4118:
						if (src.bufQueue.length === 1 && src.bufQueue[0].id === 0 || src.looping) return 0;
					...
					case 4133:
						...
						if (offset > 0) offset *= src.bufQueue[0].frequency;
						return offset;
					case 4134:
						var offset = AL.sourceTell(src);
						if (offset > 0) offset *= src.bufQueue[0].frequency * src.bufQueue[0].bytesPerSample;
						return offset;
					...
					case 8201:
						...
						for (var i = 0; i < src.bufQueue.length; i++) {
							...
							if (src.bufQueue[i].id !== 0) bytesPerFrame = src.bufQueue[i].bytesPerSample * src.bufQueue[i].channels;
						}
						return length * bytesPerFrame;
					case 8202:
						var length = 0;
						for (var i = 0; i < src.bufQueue.length; i++) length += src.bufQueue[i].length;
						return length;
			...
			setSourceParam: (funcname, sourceId, param, value) => {
				...
				switch (param) {
					...
					case 4097:
						...
						if (src.panner) src.panner.coneInnerAngle = value % 360;
						break;
					case 4098:
						if (!Number.isFinite(value)) {
							AL.currentCtx.err = 40963;
						...
						if (src.panner) src.panner.coneOuterAngle = value % 360;
						break;
					case 4099:
						if (!Number.isFinite(value) || value <= 0) {
							AL.currentCtx.err = 40963;
					...
					case 4105:
						...
						if (value === 0) {
							for (var i in src.bufQueue) src.bufQueue[i].refCount--;
					...
					case 4128:
						...
						if (src.panner) src.panner.refDistance = value;
						break;
					case 4129:
						if (!Number.isFinite(value) || value < 0) {
							AL.currentCtx.err = 40963;
						...
						if (src.panner) src.panner.rolloffFactor = value;
						break;
					case 4130:
						...
						if (src.panner) src.panner.coneOuterGain = value;
						break;
					case 4131:
						if (!Number.isFinite(value) || value < 0) {
							AL.currentCtx.err = 40963;
						...
						if (src.panner) src.panner.maxDistance = value;
						break;
					case 4132:
					...
					case 4133:
						...
						if (srcLen > 0) {
							var frequency;
							for (var bufId in src.bufQueue) if (bufId) {
					...
					case 53248:
						switch (value) {
							...
							case 53254:
								...
								if (AL.currentCtx.sourceDistanceModel) AL.updateContextGlobal(AL.currentCtx);
								break;
							default:
								AL.currentCtx.err = 40963;
								return;
				...
				switch (format) {
					case 4352:
						if (size > 0) {
							...
							for (var i = 0; i < size; ++i) channel0[i] = HEAPU8[pData++] * .0078125 - 1;
						}
					...
					case 4353:
						if (size > 0) {
							...
							var channel0 = audioBuf.getChannelData(0);
							pData >>= 1;
							for (var i = 0; i < size >> 1; ++i) channel0[i] = HEAP16[pData++] * 30517578125e-15;
						}
		...
		var _alGetBuffer3f = (bufferId, param, pValue0, pValue1, pValue2) => {
			if (AL.getBufferParam("alGetBuffer3f", bufferId, param) === null) return;
		...
		var _alGetBuffer3i = (bufferId, param, pValue0, pValue1, pValue2) => {
			if (AL.getBufferParam("alGetBuffer3i", bufferId, param) === null) return;
			if (!pValue0 || !pValue1 || !pValue2) {
				AL.currentCtx.err = 40963;
				return;
			}
		...
		var _alGetBufferf = (bufferId, param, pValue) => {
			if (AL.getBufferParam("alGetBufferf", bufferId, param) === null) return;
			if (!pValue) {
				AL.currentCtx.err = 40963;
				return;
			}
		...
		var _alGetBufferfv = (bufferId, param, pValues) => {
			if (AL.getBufferParam("alGetBufferfv", bufferId, param) === null) return;
			if (!pValues) {
				AL.currentCtx.err = 40963;
				return;
			}
		...
		var _alGetEnumValue = (pEnumName) => {
			if (!AL.currentCtx) return 0;
			if (!pEnumName) {
				AL.currentCtx.err = 40963;
				return 0;
			}
			switch (UTF8ToString(pEnumName)) {
		...
		var _alGetListeneri = (param, pValue) => {
			if (AL.getListenerParam("alGetListeneri", param) === null) return;
			if (!pValue) {
				AL.currentCtx.err = 40963;
				return;
			}
		...
		var _alGetString = (param) => {
			...
			switch (param) {
				...
				case 45060:
					...
					break;
				default:
					if (AL.currentCtx) AL.currentCtx.err = 40962;
					return 0;
			}
		...
		var _alIsBuffer = (bufferId) => {
			if (!AL.currentCtx) return false;
			if (bufferId > AL.buffers.length) return false;
			if (!AL.buffers[bufferId]) return false;
			return true;
		};
		_alIsBuffer.sig = "ii";
		...
		var _alIsSource = (sourceId) => {
			if (!AL.currentCtx) return false;
			if (!AL.currentCtx.sources[sourceId]) return false;
			return true;
		};
		_alIsSource.sig = "ii";
		...
		var _alSourcePausev = (count, pSourceIds) => {
			if (!AL.currentCtx) return;
			if (!pSourceIds) AL.currentCtx.err = 40963;
			for (var i = 0; i < count; ++i) if (!AL.currentCtx.sources[HEAP32[pSourceIds + i * 4 >> 2]]) {
				AL.currentCtx.err = 40961;
				return;
			}
			for (var i = 0; i < count; ++i) {
		...
		var _alSourceQueueBuffers = (sourceId, count, pBufferIds) => {
			...
			if (count === 0) return;
			var templateBuf = AL.buffers[0];
			for (var buf of src.bufQueue) if (buf.id !== 0) {
				templateBuf = buf;
				break;
			}
			for (var i = 0; i < count; ++i) {
				...
				if (templateBuf.id !== 0 && (buf.frequency !== templateBuf.frequency || buf.bytesPerSample !== templateBuf.bytesPerSample || buf.channels !== templateBuf.channels)) AL.currentCtx.err = 40964;
			}
			if (src.bufQueue.length === 1 && src.bufQueue[0].id === 0) src.bufQueue.length = 0;
			src.type = 4137;
			for (var i = 0; i < count; ++i) {
				var bufId = HEAP32[pBufferIds + i * 4 >> 2];
				var buf = AL.buffers[bufId];
			...
			if (src.looping) AL.cancelPendingSourceAudio(src);
			AL.initSourcePanner(src);
			AL.scheduleSourceAudio(src);
		};
		...
		var _alSourceUnqueueBuffers = (sourceId, count, pBufferIds) => {
			...
			if (src.bufQueue.length === 0) src.bufQueue.push(AL.buffers[0]);
			AL.initSourcePanner(src);
			AL.scheduleSourceAudio(src);
		};
		...
		var _alcCaptureCloseDevice = (deviceId) => {
			...
			if (c.mediaStream) c.mediaStream.getTracks().forEach((track) => track.stop());
			delete c.buffers;
		...
		var autoResumeAudioContext = (ctx) => {
			for (var event of [
				...
				"mousedown",
				"touchstart"
			]) for (var element of [document, document.getElementById("canvas")]) element?.addEventListener(event, () => {
		...
		var _alcCaptureOpenDevice = (pDeviceName, requestedSampleRate, format, bufferFrameCapacity) => {
			...
			if (!AL.sharedCaptureAudioCtx) try {
			...
			var buffers = [];
			try {
				for (var chan = 0; chan < outputChannelCount; ++chan) buffers[chan] = newSampleArray(bufferFrameCapacity);
			} catch (e) {
				AL.alcErr = 40965;
				return 0;
			}
			...
			var onSuccess = (mediaStream) => {
				...
				newCapture.scriptProcessorNode = newCapture.audioCtx.createScriptProcessor(512, inputChannelCount, outputChannelCount);
				...
				} else newCapture.mediaStreamSourceNode.connect(newCapture.scriptProcessorNode);
				...
				newCapture.scriptProcessorNode.onaudioprocess = (audioProcessingEvent) => {
					if (!newCapture.isCapturing) return;
					var c = newCapture;
			...
			if (navigator.mediaDevices?.getUserMedia) navigator.mediaDevices.getUserMedia({ audio: true }).then(onSuccess).catch(onError);
			else navigator.getUserMedia({ audio: true }, onSuccess, onError);
			var id = AL.newId();
		...
		var _alcCaptureSamples = (deviceId, pFrames, requestedFrameCount) => {
			...
			var fratio = c.audioCtx.sampleRate / dstfreq;
			...
			if (Math.floor(fratio) == fratio) for (var i = 0, frame_i = 0; frame_i < requestedFrameCount; ++frame_i) {
				for (var chan = 0; chan < c.buffers.length; ++chan, ++i) setSample(i, c.buffers[chan][c.captureReadhead]);
				c.captureReadhead = (fratio + c.captureReadhead) % c.bufferFrameCapacity;
			}
			else for (var i = 0, frame_i = 0; frame_i < requestedFrameCount; ++frame_i) {
		...
		var _alcCreateContext = (deviceId, pAttrList) => {
			...
			var ac = null;
			try {
				if (options) ac = new AudioContext(options);
				else ac = new AudioContext();
			} catch (e) {
				if (e.name === "NotSupportedError") AL.alcErr = 40964;
				else AL.alcErr = 40961;
				return 0;
			}
			...
			var ctx = {
				...
				set err(val) {
					if (this._err === 0 || val === 0) this._err = val;
				}
			};
			AL.deviceRefCounts[deviceId]++;
			AL.contexts[ctx.id] = ctx;
			if (hrtf !== null) for (var ctxId in AL.contexts) {
		...
		var _alcDestroyContext = (contextId) => {
			...
			if (AL.contexts[contextId].interval) clearInterval(AL.contexts[contextId].interval);
		...
		var _alcGetContextsDevice = (contextId) => {
			if (contextId in AL.contexts) return AL.contexts[contextId].deviceId;
			return 0;
		};
		_alcGetContextsDevice.sig = "pp";
		var _alcGetCurrentContext = () => {
			if (AL.currentCtx !== null) return AL.currentCtx.id;
			return 0;
		};
		_alcGetCurrentContext.sig = "p";
		var _alcGetEnumValue = (deviceId, pEnumName) => {
			if (deviceId !== 0 && !(deviceId in AL.deviceRefCounts)) return 0;
			else if (!pEnumName) {
				AL.alcErr = 40964;
				return 0;
			}
		...
		var _alcGetIntegerv = (deviceId, param, size, pValues) => {
			...
			switch (param) {
				...
				case 4099:
					...
					for (var i = 0; i < AL.currentCtx.attrs.length; i++) HEAP32[pValues + i * 4 >> 2] = AL.currentCtx.attrs[i];
					break;
				case 4103:
					if (!(deviceId in AL.deviceRefCounts)) {
						AL.alcErr = 40961;
				...
				case 6547:
					...
					for (var ctxId in AL.contexts) {
						...
						if (ctx.deviceId === deviceId) hrtfStatus = ctx.hrtf ? 1 : 0;
					}
		...
		var _alcGetString = (deviceId, param) => {
			...
			switch (param) {
				...
				case 40965:
					...
					break;
				case 4100:
					if (globalThis.AudioContext || globalThis.webkitAudioContext) ret = AL.DEVICE_NAME;
					else return 0;
					break;
				case 4101:
					if (globalThis.AudioContext || globalThis.webkitAudioContext) ret = AL.DEVICE_NAME + "\0";
					else ret = "\0";
					break;
				case 785:
					...
					break;
				case 784:
					if (deviceId === 0) ret = AL.CAPTURE_DEVICE_NAME + "\0";
					else {
		...
		var _alcMakeContextCurrent = (contextId) => {
			if (contextId === 0) AL.currentCtx = null;
			else AL.currentCtx = AL.contexts[contextId];
			return 1;
		};
		_alcMakeContextCurrent.sig = "ip";
		var _alcOpenDevice = (pDeviceName) => {
			if (pDeviceName) {
				if (UTF8ToString(pDeviceName) !== AL.DEVICE_NAME) return 0;
			}
		...
		var _clock_res_get = (clk_id, pres) => {
			...
			var nsec;
			if (clk_id === 0) nsec = 1e3 * 1e3;
			else if (nowIsMonotonic) nsec = _emscripten_get_now_res();
			else return 52;
		...
		var GL = {
			...
			newRenderingFrameStarted: () => {
				...
				var largestIndex = GL.log2ceilLookup(GL.MAX_TEMP_BUFFER_SIZE);
				for (var i = 0; i <= largestIndex; ++i) GL.currentContext.tempVertexBufferCounters1[i] = 0;
			},
			...
			calcBufLength: (size, type, stride, count) => {
				...
				return size * GL.byteSizeByType[type - GL.byteSizeByTypeRoot] * count;
			},
			usedTempBuffers: [],
			...
			registerContext: (ctx, webGLContextAttributes) => {
				...
				for (var i = 0; i < context.maxVertexAttribs; i++) context.clientBuffers[i] = {
					enabled: false,
					clientside: false,
					size: 0,
					type: 0,
			...
			initExtensions: (context) => {
				...
				for (var ext of getEmscriptenSupportedExtensions(GLctx)) if (!ext.includes("lose_context") && !ext.includes("debug")) GLctx.getExtension(ext);
			}
		};
		...
		var _emscripten_alcDevicePauseSOFT = (deviceId) => {
			...
			for (var ctxId in AL.contexts) {
				var ctx = AL.contexts[ctxId];
				if (ctx.deviceId !== deviceId) continue;
		...
		var getCanvasElementSize = (target) => {
			...
			var w = stackAlloc(8);
			var h = w + 4;
			var ret = _emscripten_get_canvas_element_size(stringToUTF8OnStack(target.id), w, h);
		...
		var setCanvasElementSize = (target, width, height) => {
			if (!target.controlTransferredOffscreen) {
				...
				var sp = stackSave();
				_emscripten_set_canvas_element_size(stringToUTF8OnStack(target.id), width, height);
				stackRestore(sp);
			}
		};
		...
		var registerRestoreOldStyle = (canvas) => {
			...
			function restoreOldStyle() {
				if (!getFullscreenElement()) {
					...
					if (currentFullscreenStrategy.canvasResizedCallback) getWasmTableEntry(currentFullscreenStrategy.canvasResizedCallback)(37, 0, currentFullscreenStrategy.canvasResizedCallbackUserData);
				}
			}
		...
		var JSEvents_resizeCanvasForFullscreen = (target, strategy) => {
			...
			(_a = target.style).backgroundColor || (_a.backgroundColor = "black");
			(_b = document.body.style).backgroundColor || (_b.backgroundColor = "black");
		...
		var JSEvents_requestFullscreen = (target, strategy) => {
			...
			if (strategy.canvasResizedCallback) getWasmTableEntry(strategy.canvasResizedCallback)(37, 0, strategy.canvasResizedCallbackUserData);
			return 0;
		};
		...
		var fillGamepadEventData = (eventStruct, e) => {
			...
			for (var i = 0; i < e.buttons.length; ++i) if (typeof e.buttons[i] == "object") HEAPF64[eventStruct + i * 8 + 528 >> 3] = e.buttons[i].value;
			else HEAPF64[eventStruct + i * 8 + 528 >> 3] = e.buttons[i];
			for (var i = 0; i < e.buttons.length; ++i) if (typeof e.buttons[i] == "object") HEAP8[eventStruct + i + 1040] = e.buttons[i].pressed;
			else HEAP8[eventStruct + i + 1040] = e.buttons[i] == 1;
		...
		var _emscripten_glDrawElements = (mode, count, type, indices) => {
			...
			if (!GLctx.currentElementArrayBufferBinding) {
				...
				if (count > 0) for (var i = 0; i < GL.currentContext.maxVertexAttribs; ++i) {
					var cb = GL.currentContext.clientBuffers[i];
		...
		var _emscripten_glGetProgramiv = (program, pname, p) => {
			...
			if (pname == 35716) {
				...
				if (!program.maxUniformLength) {
					...
					for (var i = 0; i < numActiveUniforms; ++i) program.maxUniformLength = Math.max(program.maxUniformLength, GLctx.getActiveUniform(program, i).name.length + 1);
				}
				HEAP32[p >> 2] = program.maxUniformLength;
			} else if (pname == 35722) {
				if (!program.maxAttributeLength) {
					var numActiveAttributes = GLctx.getProgramParameter(program, 35721);
					for (var i = 0; i < numActiveAttributes; ++i) program.maxAttributeLength = Math.max(program.maxAttributeLength, GLctx.getActiveAttrib(program, i).name.length + 1);
				}
				HEAP32[p >> 2] = program.maxAttributeLength;
			} else if (pname == 35381) {
				if (!program.maxUniformBlockNameLength) {
					var numActiveUniformBlocks = GLctx.getProgramParameter(program, 35382);
					for (var i = 0; i < numActiveUniformBlocks; ++i) program.maxUniformBlockNameLength = Math.max(program.maxUniformBlockNameLength, GLctx.getActiveUniformBlockName(program, i).length + 1);
				}
		...
		var computeUnpackAlignedImageSize = (width, height, sizePerPixel) => {
			...
			return height * roundedToNextMultipleOf((GL.unpackRowLength || width) * sizePerPixel, GL.unpackAlignment);
		};
		...
		var emscriptenWebGLGetTexPixelData = (type, format, width, height, pixels, internalFormat) => {
			var heap = heapObjectForWebGLType(type);
			var bytes = computeUnpackAlignedImageSize(width, height, colorChannelsInGlTextureFormat(format) * heap.BYTES_PER_ELEMENT);
		...
		var _emscripten_net_bind = (fd, sockaddr, socklen) => Module.net?.bind?.(fd, sockaddr, socklen) ?? -1;
		var _emscripten_net_closesocket = (fd) => Module.net?.closesocket?.(fd) ?? -1;
		var _emscripten_net_getaddrinfo = (hostnamePtr, restrictPrt, hintsPtr, addrinfoPtr) => Module.net?.getaddrinfo?.(hostnamePtr, restrictPrt, hintsPtr, addrinfoPtr) ?? -1;
		var _emscripten_net_gethostname = (name, namelen) => Module.net?.gethostname?.(name, namelen) ?? -1;
		var _emscripten_net_getsockname = (fd, sockaddr, socklen) => Module.net?.getsockname?.(fd, sockaddr, socklen) ?? -1;
		var _emscripten_net_recvfrom = (fd, buf, len, flags, sockaddr, socklen) => Module.net?.recvfrom?.(fd, buf, len, flags, sockaddr, socklen) ?? -1;
		var _emscripten_net_sendto = (fd, buf, len, flags, sockaddr, socklen) => Module.net?.sendto?.(fd, buf, len, flags, sockaddr, socklen) ?? -1;
		var _emscripten_net_sendto_batch = (fd, bufs, lens, flags, sockaddr, socklen) => Module.net?.sendto_batch?.(fd, bufs, lens, flags, sockaddr, socklen) ?? -1;
		var _emscripten_net_socket = (family, type, protocol) => Module.net?.socket?.(family, type, protocol) ?? -1;
		...
		var _emscripten_request_fullscreen_strategy = (target, deferUntilInEventHandler, fullscreenStrategy) => {
			return doRequestFullscreen(target, {
		...
		function _fd_pwrite(fd, iov, iovcnt, offset, pnum) {
			...
			try {
				if (isNaN(offset)) return 61;
				var num = doWritev(SYSCALLS.getStreamFromFD(fd), iov, iovcnt, offset);
				HEAPU32[pnum >> 2] = num;
				return 0;
			} catch (e) {
				if (typeof FS == "undefined" || !(e.name === "ErrnoError")) throw e;
		...
		var writeI53ToI64Clamped = (ptr, num) => {
			...
			} else writeI53ToI64(ptr, num);
		};
		var writeI53ToI64Signaling = (ptr, num) => {
			if (num > 0x8000000000000000 || num < -0x8000000000000000) throw `RangeError: ${num}`;
		...
		var writeI53ToU64Signaling = (ptr, num) => {
			if (num < 0 || num > 0x10000000000000000) throw `RangeError: ${num}`;
			writeI53ToI64(ptr, num);
		};
		...
		var _getaddrinfo = (node, service, hint, out) => {
			...
			function allocaddrinfo(family, type, proto, canon, addr, port) {
				...
				sa = _malloc(salen);
				writeSockaddr(sa, family, addr, port);
				ai = _malloc(32);
			...
			if (!node) {
				...
				if ((flags & 1) === 0) if (family === 2) addr = _htonl(2130706433);
				else addr = [
					0,
					0,
					0,
			...
			addr = inetPton4(node);
			if (addr !== null) if (family === 0 || family === 2) family = 2;
				...
				addr = inetPton6(node);
				if (addr !== null) if (family === 0 || family === 10) family = 10;
				else return -2;
			}
			if (addr != null) {
		...
		var _getprotoent = (number) => {
			if (_setprotoent.index === Protocols.list.length) return 0;
			return Protocols.list[_setprotoent.index++];
		};
		_getprotoent.sig = "p";
		...
		var _emscripten_asm_const_double_sync_on_main_thread = (a1, a2, a3) => _emscripten_asm_const_int_sync_on_main_thread(a1, a2, a3);
		...
		var __Unwind_Backtrace = (func, arg) => {
			var parts = getCallstack().split("\n");
			for (var i = 0; i < parts.length; i++) if (getWasmTableEntry(func)(0, arg) !== 0) return;
		};
		...
		var _emscripten_math_hypot = (count, varargs) => {
			var args = [];
			for (var i = 0; i < count; ++i) args.push(HEAPF64[varargs + i * 8 >> 3]);
		...
		_emscripten_html5_remove_event_listener.sig = "ippip";
		...
		var screenOrientation = () => {
			if (!window.screen) return void 0;
		...
		var fillOrientationChangeEventData = (eventStruct) => {
			...
			if (typeof screenOrientObj === "object") {
				...
				if (orientationIndex < 0) orientationIndex = orientationsType2.indexOf(screenOrientObj.type);
				if (orientationIndex >= 0) orientationIndex = 1 << orientationIndex;
				orientationAngle = screenOrientObj.angle;
			} else orientationAngle = window.orientation;
		...
		var _emscripten_lock_orientation = (allowedOrientations) => {
			...
			var succeeded;
			if (screen.lockOrientation) succeeded = screen.lockOrientation(orientations);
			else if (screen.mozLockOrientation) succeeded = screen.mozLockOrientation(orientations);
			else if (screen.webkitLockOrientation) succeeded = screen.webkitLockOrientation(orientations);
			else return -1;
		...
		var hideEverythingExceptGivenElement = (onlyVisibleElement) => {
			...
			while (child != document.body) {
				for (var currChild of parent.children) if (currChild != child) {
					hiddenElements.push({
						node: currChild,
		...
		var restoreHiddenElements = (hiddenElements) => {
			for (var elem of hiddenElements) elem.node.style.display = elem.displayState;
		};
		...
		var softFullscreenResizeWebGLRenderTarget = () => {
			...
			if (!inCenteredWithoutScalingFullscreenMode && currentFullscreenStrategy.canvasResizedCallback) getWasmTableEntry(currentFullscreenStrategy.canvasResizedCallback)(37, 0, currentFullscreenStrategy.canvasResizedCallbackUserData);
		};
		...
		var _emscripten_get_visibility_status = (visibilityStatus) => {
			if (typeof document.visibilityState == "undefined" && typeof document.hidden == "undefined") return -1;
		...
		var _emscripten_request_animation_frame_loop = (cb, userData) => {
			function tick(timeStamp) {
				if (getWasmTableEntry(cb)(timeStamp, userData)) requestAnimationFrame(tick);
			}
		...
		var _emscripten_get_callstack = (flags, str, maxbytes) => {
			...
			if (!str || maxbytes <= 0) return lengthBytesUTF8(callstack) + 1;
			return stringToUTF8(callstack, str, maxbytes) + 1;
		};
		_emscripten_get_callstack.sig = "iipi";
		var convertFrameToPC = (frame) => {
			var match;
			if (match = /\bwasm-function\[\d+\]:(0x[0-9a-f]+)/.exec(frame)) return +match[1];
			else if (match = /:(\d+):\d+(?:\)|$)/.exec(frame)) return 2147483648 | +match[1];
			return 0;
		};
		...
		var _emscripten_pc_get_function = (pc) => {
			...
			var name;
			var match;
			if (match = /^\s+at .*\.wasm\.(.*) \(.*\)$/.exec(frame)) name = match[1];
			else if (match = /^\s+at (.*) \(.*\)$/.exec(frame)) name = match[1];
			else if (match = /^(.+?)@/.exec(frame)) name = match[1];
			else return 0;
		...
		var convertPCtoSourceLocation = (pc) => {
			...
			if (!source) {
				...
				if (match = /\((.*):(\d+):(\d+)\)$/.exec(frame)) source = {
					file: match[1],
					line: match[2],
					column: match[3]
				};
				else if (match = /@(.*):(\d+):(\d+)/.exec(frame)) source = {
					file: match[1],
					line: match[2],
					column: match[3]
				};
		...
		var wasiRightsToMuslOFlags = (rights) => {
			if (rights & 2 && rights & 64) return 2;
			if (rights & 2) return 0;
			if (rights & 64) return 1;
			throw new FS.ErrnoError(28);
		};
		var wasiOFlagsToMuslOFlags = (oflags) => {
			var musl_oflags = 0;
			if (oflags & 1) musl_oflags |= 64;
			if (oflags & 8) musl_oflags |= 512;
			if (oflags & 2) musl_oflags |= 65536;
			if (oflags & 4) musl_oflags |= 128;
			return musl_oflags;
		};
		...
		var _emscripten_set_immediate_loop = (cb, userData) => {
			function tick() {
				callUserCallback(() => {
					if (getWasmTableEntry(cb)(userData)) emSetImmediate(tick);
				});
			}
			emSetImmediate(tick);
		};
		...
		var _emscripten_async_call = (func, arg, millis) => {
			...
			if (millis >= 0) safeSetTimeout(wrapper, millis);
			else safeRequestAnimationFrame(wrapper);
		};
		...
		var _emscripten_promise_then = (id, onFulfilled, onRejected, userData) => {
			...
			return promiseMap.allocate({ promise: promise.then(makePromiseCallback(onFulfilled, userData), makePromiseCallback(onRejected, userData)) });
		};
		_emscripten_promise_then.sig = "ppppp";
		var _emscripten_promise_all = (idBuf, resultBuf, size) => {
			var promises = idsToPromises(idBuf, size);
			return promiseMap.allocate({ promise: Promise.all(promises).then((results) => {
				if (resultBuf) for (var i = 0; i < size; i++) {
		...
		var _emscripten_promise_all_settled = (idBuf, resultBuf, size) => {
			var promises = idsToPromises(idBuf, size);
			return promiseMap.allocate({ promise: Promise.allSettled(promises).then((results) => {
				if (resultBuf) {
					var offset = resultBuf;
					for (var i = 0; i < size; i++, offset += 8) if (results[i].status === "fulfilled") setPromiseResult(offset, true, results[i].value);
					else setPromiseResult(offset, false, results[i].reason);
				}
				return resultBuf;
			}) });
		};
		...
		var _emscripten_promise_any = (idBuf, errorBuf, size) => {
			var promises = idsToPromises(idBuf, size);
			return promiseMap.allocate({ promise: Promise.any(promises).catch((err) => {
				if (errorBuf) for (var i = 0; i < size; i++) HEAPU32[errorBuf + i * 4 >> 2] = err.errors[i];
				throw errorBuf;
			}) });
		};
		...
		var _emscripten_promise_race = (idBuf, size) => {
			var promises = idsToPromises(idBuf, size);
			return promiseMap.allocate({ promise: Promise.race(promises) });
		};
		...
		function ___cxa_current_exception_type() {
			if (!exceptionCaught.length) return 0;
			return exceptionCaught[exceptionCaught.length - 1].get_type();
		}
		...
		var _emscripten_async_load_script = async (url, onload, onerror) => {
			...
			var loadDone = () => {
				if (onload) {
					...
					if (runDependencies > 0) dependenciesFulfilled = onloadCallback;
					else onloadCallback();
				}
			};
			var loadError = () => {
				if (onerror) callUserCallback(getWasmTableEntry(onerror));
			};
		...
		var _emscripten_get_window_title = () => {
			var buflen = 256;
			if (!_emscripten_get_window_title.buffer) _emscripten_get_window_title.buffer = _malloc(buflen);
		...
		var _emscripten_hide_mouse = () => {
			...
			for (var i = 0; i < rules.length; i++) if (rules[i].cssText.startsWith("canvas")) {
		...
		var _emscripten_create_worker = (url) => {
			...
			info.worker.onmessage = function info_worker_onmessage(msg) {
				...
				} else callbackInfo.func(0, 0, callbackInfo.arg);
			};
		...
		var _emscripten_call_worker = (id, funcName, data, size, callback, arg) => {
			...
			if (data) info.worker.postMessage(transferObject, [transferObject.data.buffer]);
			else info.worker.postMessage(transferObject);
		};
		...
		var _emscripten_get_preloaded_image_data_from_FILE = (file, w, h) => {
			var fd = _fileno(file);
			var stream = FS.getStream(fd);
			if (stream) return getPreloadedImageData(stream.path, w, h);
			return 0;
		};
		...
		var _emscripten_async_wget = (url, file, onload, onerror) => {
			...
			function doCallback(callback) {
				if (callback) callUserCallback(() => withStackSave(() => getWasmTableEntry(callback)(stringToUTF8OnStack(_file))));
			}
			var destinationDirectory = PATH.dirname(_file);
			FS_preloadFile(destinationDirectory, PATH.basename(_file), _url, true, true, false, false, () => {
				try {
					FS_unlink(_file);
				} catch (e) {}
		...
		var _emscripten_async_wget2 = (url, file, request, param, userdata, onload, onerror, onprogress) => {
			...
			http.onload = (e) => {
				...
				} else if (onerror) getWasmTableEntry(onerror)(handle, userdata, http.status);
		...
		var _emscripten_async_wget2_data = (url, request, param, userdata, free, onload, onerror, onprogress) => {
			...
			function onerrorjs() {
				if (onerror) {
					var sp = stackSave();
					var statusText = 0;
					if (http.statusText) statusText = stringToUTF8OnStack(http.statusText);
			...
			http.onprogress = (e) => {
				if (onprogress) getWasmTableEntry(onprogress)(handle, userdata, e.loaded, e.lengthComputable || e.lengthComputable === void 0 ? e.total : 0);
			};
			http.onabort = (e) => {
				delete wget.wgetRequests[handle];
			};
		...
		var _emscripten_async_wget2_abort = (handle) => {
			wget.wgetRequests[handle]?.abort();
		};
		...
		var ___asctime_r = (tmPtr, buf) => {
			...
			stringToUTF8([
				...
				"Nov",
				"Dec"
			][date.tm_mon] + (date.tm_mday < 10 ? "  " : " ") + date.tm_mday + (date.tm_hour < 10 ? " 0" : " ") + date.tm_hour + (date.tm_min < 10 ? ":0" : ":") + date.tm_min + (date.tm_sec < 10 ? ":0" : ":") + date.tm_sec + " " + (1900 + date.tm_year) + "\n", buf, 26);
			return buf;
		};
		...
		var _strptime = (buf, format, tm) => {
			...
			var SPECIAL_CHARS = "\\!@#$^&*()+=-[]/{}|:<>?,.";
			for (var i = 0, ii = SPECIAL_CHARS.length; i < ii; ++i) pattern = pattern.replace(new RegExp("\\" + SPECIAL_CHARS[i], "g"), "\\" + SPECIAL_CHARS[i]);
			...
			if (matches) {
				...
				var getMatch = (symbol) => {
					...
					if (pos >= 0) return matches[pos + 1];
				};
				if (value = getMatch("S")) date.sec = Number(value);
				if (value = getMatch("M")) date.min = Number(value);
				if (value = getMatch("H")) date.hour = Number(value);
				else if (value = getMatch("I")) {
					var hour = Number(value);
					if (value = getMatch("p")) hour += value.toUpperCase()[0] === "P" ? 12 : 0;
					date.hour = hour;
				}
				if (value = getMatch("Y")) date.year = Number(value);
				else if (value = getMatch("y")) {
					var year = Number(value);
					if (value = getMatch("C")) year += Number(value) * 100;
					else year += year < 69 ? 2e3 : 1900;
					date.year = year;
				}
				if (value = getMatch("m")) date.month = Number(value) - 1;
				else if (value = getMatch("b")) date.month = MONTH_NUMBERS[value.substring(0, 3).toUpperCase()] || 0;
				if (value = getMatch("d")) date.day = Number(value);
				else if (value = getMatch("j")) {
					var day = Number(value);
					var leapYear = isLeapYear(date.year);
					for (var month = 0; month < 12; ++month) {
						var daysUntilMonth = arraySum(leapYear ? MONTH_DAYS_LEAP : MONTH_DAYS_REGULAR, month - 1);
						if (day <= daysUntilMonth + (leapYear ? MONTH_DAYS_LEAP : MONTH_DAYS_REGULAR)[month]) date.day = day - daysUntilMonth;
					}
					...
					if (value = getMatch("U")) {
						...
						var endDate;
						if (janFirst.getDay() === 0) endDate = addDays(janFirst, weekDayNumber + 7 * (weekNumber - 1));
						else endDate = addDays(janFirst, 7 - janFirst.getDay() + weekDayNumber + 7 * (weekNumber - 1));
						...
						var janFirst = new Date(date.year, 0, 1);
						var endDate;
						if (janFirst.getDay() === 1) endDate = addDays(janFirst, weekDayNumber + 7 * (weekNumber - 1));
						else endDate = addDays(janFirst, 7 - janFirst.getDay() + 1 + weekDayNumber + 7 * (weekNumber - 1));
						date.day = endDate.getDate();
						date.month = endDate.getMonth();
					}
				}
				if (value = getMatch("z")) if (value.toLowerCase() === "z") date.gmtoff = 0;
				else {
					var match = value.match(/^((?:\-|\+)\d\d):?(\d\d)?/);
					date.gmtoff = match[1] * 3600;
					if (match[2]) date.gmtoff += date.gmtoff > 0 ? match[2] * 60 : -match[2] * 60;
				}
		...
		var __dlsym_catchup_js = (handle, symbolIndex) => {
			var symDict = LDSO.loadedLibsByHandle[handle].exports;
			var sym = symDict[Object.keys(symDict)[symbolIndex]];
			return addFunction(sym, sym.sig);
		};
		...
		var _setNetworkCallback = (event, userData, callback) => {
			function _callback(data) {
				callUserCallback(() => {
					if (event === "error") withStackSave(() => {
						var msg = stringToUTF8OnStack(data[2]);
						getWasmTableEntry(callback)(data[0], data[1], msg, userData);
					});
					else getWasmTableEntry(callback)(data, userData);
				});
			}
		...
		var _eglGetCurrentSurface = (readdraw) => {
			if (readdraw == 12378) return EGL.currentReadSurface;
			else if (readdraw == 12377) return EGL.currentDrawSurface;
			else {
				EGL.setErrorCode(12300);
				return 0;
			}
		...
		var _emscripten_glVertexPointer = (size, type, stride, ptr) => abort("Legacy GL function (glVertexPointer) called. If you want legacy GL emulation, you need to compile with -sLEGACY_GL_EMULATION to enable legacy GL emulation.");
		_emscripten_glVertexPointer.sig = "viiip";
		var _glVertexPointer = _emscripten_glVertexPointer;
		_glVertexPointer.sig = "viiip";
		var _emscripten_glMatrixMode = () => abort("Legacy GL function (glMatrixMode) called. If you want legacy GL emulation, you need to compile with -sLEGACY_GL_EMULATION to enable legacy GL emulation.");
		_emscripten_glMatrixMode.sig = "vi";
		var _glMatrixMode = _emscripten_glMatrixMode;
		_glMatrixMode.sig = "vi";
		var _emscripten_glBegin = () => abort("Legacy GL function (glBegin) called. If you want legacy GL emulation, you need to compile with -sLEGACY_GL_EMULATION to enable legacy GL emulation.");
		_emscripten_glBegin.sig = "vi";
		var _glBegin = _emscripten_glBegin;
		_glBegin.sig = "vi";
		var _emscripten_glLoadIdentity = () => abort("Legacy GL function (glLoadIdentity) called. If you want legacy GL emulation, you need to compile with -sLEGACY_GL_EMULATION to enable legacy GL emulation.");
		...
		var writeGLArray = (arr, dst, dstLength, heapType) => {
			...
			dst = dst >> 2;
			for (var i = 0; i < writeLength; ++i) heap[dst + i] = arr[i];
			return len;
		};
		var webglPowerPreferences = [
			"default",
			"low-power",
			"high-performance"
		];
		var _emscripten_webgl_do_create_context = (target, attributes) => {
			...
			var powerPreference = HEAP32[attr32 + 2];
			var contextAttributes = {
				...
				majorVersion: HEAP32[attr32 + 4],
				minorVersion: HEAP32[attr32 + 5],
				enableExtensionsByDefault: HEAP8[attributes + 24],
				explicitSwapControl: HEAP8[attributes + 25],
				proxyContextToMainThread: HEAP32[attr32 + 7],
		...
		var _emscripten_webgl_make_context_current = (contextHandle) => {
			return GL.makeContextCurrent(contextHandle) ? 0 : -5;
		};
		...
		var _emscripten_webgl_get_drawing_buffer_size = (contextHandle, width, height) => {
			...
			if (!GLContext || !GLContext.GLctx || !width || !height) return -5;
		...
		var _emscripten_webgl_enable_extension = (contextHandle, extension) => {
			...
			return !!context.GLctx.getExtension(extString);
		};
		...
		var _emscripten_webgl_get_vertex_attrib_o = (index, param) => {
			return GLctx.getVertexAttrib(index, param)?.name;
		};
		...
		var _emscripten_webgl_get_parameter_o = (param) => {
			return GLctx.getParameter(param)?.name;
		};
		...
		_emscripten_glDrawArraysInstancedBaseInstanceWEBGL.sig = "viiiii";
		...
		_emscripten_glDrawElementsInstancedBaseVertexBaseInstanceWEBGL.sig = "viiiiiii";
		...
		_emscripten_glMultiDrawElementsInstancedBaseVertexBaseInstanceWEBGL.sig = "viiiiiiii";
		...
		var allocate = (slab, allocator) => {
			...
			if (!slab.subarray && !slab.slice) slab = new Uint8Array(slab);
			HEAPU8.set(slab, ret);
			return ret;
		};
		...
		function getNativeTypeSize(type) {
			switch (type) {
				...
				default:
					if (type.endsWith("*")) return POINTER_SIZE;
					if (type[0] === "i") return Number(type.slice(1)) / 8;
					return 0;
			}
		}
		...
		var _emscripten_net_gethostbyname = (hostname) => Module.net?.gethostbyname?.(hostname) ?? 0;
		var _emscripten_net_connect = (fd, sockaddr, socklen) => Module.net?.connect?.(fd, sockaddr, socklen) ?? -1;
		var _emscripten_net_send = (fd, buf, len, flags) => Module.net?.send?.(fd, buf, len, flags) ?? -1;
		var _emscripten_net_recv = (fd, buf, len, flags) => Module.net?.recv?.(fd, buf, len, flags) ?? -1;
		var _emscripten_net_select = (n, rfds, wfds, efds, tv) => Module.net?.select?.(n, rfds, wfds, efds, tv) ?? -1;
		registerWasmPlugin();
		...
		var ASM_CONSTS = {
			790556: () => {
				Module.callbacks?.gameReady?.();
			},
			790592: ($0, $1) => {
				const hInstance = $0;
				const funcJsRef = getWasmTableEntry($1).name;
				const exports = Object.entries(LDSO.loadedLibsByHandle[hInstance]?.exports ?? {});
				for (const [fn, { name }] of exports) if (name === funcJsRef) return stringToUTF8OnStack(fn);
				return 0;
			},
			790876: ($0, $1) => {
				Module.callbacks?.serverInfo?.($0, UTF8ToString($1));
			},
			790933: ($0) => {
			...
			799723: () => window.innerWidth,
			799753: () => window.innerHeight,
			799784: ($0, $1) => {
				var buf = $0;
				var buflen = $1;
				var list = void 0;
				if (navigator.languages && navigator.languages.length) list = navigator.languages;
					...
					if (oneOfThese !== void 0) list = [oneOfThese];
				}
				if (list === void 0) return;
				var str = "";
				for (var i = 0; i < list.length; i++) {
					var item = list[i];
					if (str.length + item.length + 1 > buflen) break;
					if (str.length > 0) str += ",";
					str += item;
				}
				str = str.replace(/-/g, "_");
				if (buflen > str.length) buflen = str.length;
				for (var i = 0; i < buflen; i++) setValue(buf + i, str.charCodeAt(i), "i8");
			},
			800492: ($0) => {
		...
		var _Cmd_ExecuteString, _malloc, _free, _strerror, _main, _htons, _htonl, _ntohs, _fileno, _calloc, _realloc, _getErrnoLocation, ___dl_seterr, __emscripten_find_dylib, _emscripten_builtin_memalign, __emscripten_timeout, _emscripten_get_sbrk_ptr, _setThrew, __emscripten_tempret_set, __emscripten_tempret_get, __emscripten_stack_restore, __emscripten_stack_alloc, _emscripten_stack_get_current, ___cxa_demangle, ___cxa_increment_exception_refcount, ___cxa_decrement_exception_refcount, ___cxa_can_catch, ___cxa_get_exception_ptr, memory, ___stack_pointer, __indirect_function_table, ___heap_base, wasmTable, wasmMemory;
		function assignWasmExports(wasmExports) {
			Module["_Cmd_ExecuteString"] = wasmExports["Cmd_ExecuteString"];
			_malloc = wasmExports["malloc"];
			_free = wasmExports["free"];
			_strerror = wasmExports["strerror"];
			Module["_main"] = wasmExports["__main_argc_argv"];
			...
			Module["_getErrnoLocation"] = wasmExports["getErrnoLocation"];
			...
			___heap_base = wasmExports["__heap_base"].value;
		}
		var wasmImports = {
			...
			_emscripten_get_last_devicemotion_event: __emscripten_get_last_devicemotion_event,
			_emscripten_get_last_deviceorientation_event: __emscripten_get_last_deviceorientation_event,
			_emscripten_get_last_mouse_event: __emscripten_get_last_mouse_event,
			...
			emscripten_net_bind: _emscripten_net_bind,
			emscripten_net_closesocket: _emscripten_net_closesocket,
			emscripten_net_connect: _emscripten_net_connect,
			emscripten_net_getaddrinfo: _emscripten_net_getaddrinfo,
			emscripten_net_gethostbyname: _emscripten_net_gethostbyname,
			emscripten_net_gethostname: _emscripten_net_gethostname,
			emscripten_net_getsockname: _emscripten_net_getsockname,
			emscripten_net_recv: _emscripten_net_recv,
			emscripten_net_recvfrom: _emscripten_net_recvfrom,
			emscripten_net_select: _emscripten_net_select,
			emscripten_net_send: _emscripten_net_send,
			emscripten_net_sendto: _emscripten_net_sendto,
			emscripten_net_sendto_batch: _emscripten_net_sendto_batch,
			emscripten_net_socket: _emscripten_net_socket,
		...
		function run(args = arguments_) {
			...
			function doRun() {
				...
				if (!(Module["noInitialRun"] || false)) callMain(args);
				postRun();
			}
			if (Module["setStatus"]) {
		...
		return {
			...
			start: () => {
				run();
				if (runtimeInitialized) {} else new Promise((resolve, reject) => {
...
var Xash3D = class {
	...
	*/
	constructor(opts = {}) {
		...
		this.waitTimers = /* @__PURE__ */ new Set();
		this._running = false;
		this.opts = opts;
	}
	/**
	...
	*/
	async init() {
		if (!this.emPromise) this.emPromise = this.runEm();
	...
	*/
	initRender(render = "gl4es") {
		switch (render) {
			case "gl4es":
			case "gles3compat":
				if (this.opts?.libraries?.render?.gles3compat) this.opts.filesMap[DEFAULT_GLES3COMPAT_LIBRARY] = this.opts.libraries.render.gles3compat;
				if (this.opts?.libraries?.render?.gl4es) this.opts.filesMap[DEFAULT_GL4ES_LIBRARY] = this.opts.libraries.render.gl4es;
			...
			default:
				if (this.opts?.libraries?.render?.soft) this.opts.filesMap[DEFAULT_SOFT_LIBRARY] = this.opts.libraries.render.soft;
	...
	*/
	initLibrary(library, defaultPath) {
		if (this.opts.libraries?.[library]) this.opts.filesMap[defaultPath] = this.opts.libraries[library];
	}
	/**
	...
	*/
	async getCVar(name, timeoutMs = 1e3) {
	...
	*/
	waitLog(subLog, cmd, timeoutMs = 1e3) {
		return new Promise((resolve, reject) => {
			this.waitLogs.push({
				subLog,
				resolve,
			...
			if (cmd) this.Cmd_ExecuteString(cmd);
		});
	}
	invokeWaitLogs(log) {
		const now = Date.now();
		this.waitLogs = this.waitLogs.filter((l) => {
			...
			if (now - l.createdAt > l.timeoutMs) {
				l.reject(/* @__PURE__ */ new Error("timeout"));
				return false;
			}
			return true;
		});
	...
	*/
	async runEm() {
		if (!this.opts.filesMap) this.opts.filesMap = {};
		if (!this.opts.dynamicLibraries) this.opts.dynamicLibraries = [];
		if (!this.opts.arguments) this.opts.arguments = [];
		...
		if (this.opts.libraries?.xash) this.opts.filesMap[DEFAULT_XASH_LIBRARY] = this.opts.libraries.xash;
		...
		if (this.net) this.net.init(this.em);
		this.em.FS.mkdir("/rwdir");
	}
};
//#endregion
//#region node_modules/xash3d-fwgs/dist/utils/rb.js
var RollingBuffer = class {
	...
	push(item) {
		if (this.isFull()) this.tail = (this.tail + 1) % this.opts.maxSize;
		else this.count += 1;
...
//#endregion
//#region node_modules/xash3d-fwgs/dist/net/net.js
/**
...
var Net = class {
	constructor(sender, opts = {}) {
		this.lastSocketID = 1e3;
		this.sockets = /* @__PURE__ */ new Map();
		const { maxPackets = 128, hostname = "webxash3d", hostID = 3e3 } = opts;
		this.sender = sender;
		this.opts = {
			hostname,
			maxPackets,
		...
		this.incoming = new RollingBuffer({ maxSize: maxPackets });
	}
	/**
	...
	*/
	...
	readSockaddrFast(addrPtr) {
		...
		return [ip, heapU8[portOffset] << 8 | heapU8[portOffset + 1]];
	}
	recvfrom(fd, bufPtr, bufLen, flags, sockaddrPtr, socklenPtr) {
		...
		if (copyLen > 0) em.HEAPU8.set(u8.subarray(0, copyLen), bufPtr);
		if (sockaddrPtr) {
			...
			heap16[base16] = 2;
			heap8[sockaddrPtr + 2] = port >> 8 & 255;
			heap8[sockaddrPtr + 3] = port & 255;
	...
	getaddrinfo(hostnamePtr, restrictPrt, hintsPtr, addrinfoPtr) {
		const [, identity] = this.em.AsciiToString(hostnamePtr).split(".", 2);
		const id = Number(identity);
		const sa = this.em._malloc(16);
		this.em.writeSockaddr(sa, 2, `101.101.${id >> 0 & 255}.${id >> 8 & 255}`, 0);
